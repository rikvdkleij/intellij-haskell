...
object HaskellProjectSdkSetupValidator {
  ...
  private def preparePopup(project: Project, file: VirtualFile): SdkPopupBuilder = {
    SdkPopupFactory.newBuilder.withProject(project).withSdkTypeFilter((sdkTypeId: SdkTypeId) => sdkTypeId.isInstanceOf[HaskellSdkType]).updateSdkForFile(file)
  }
}

class HaskellProjectSdkSetupValidator extends ProjectSdkSetupValidator {
  private final val Message = "Haskell SDK is not defined. Please setup Haskell SDK and reopen project."
  ...
  override def getFixHandler(project: Project, file: VirtualFile): EditorNotificationPanel.ActionHandler = {
    HaskellProjectSdkSetupValidator.preparePopup(project, file).buildEditorNotificationPanelHandler
  }
}